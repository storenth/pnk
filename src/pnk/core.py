...
class Formula:
    ...
    def pnk(self, subdomains):
        ...
        counter = 0 if self.args.cartesian else 1
        for p in itertools.permutations(subdomains):
            if counter > 0:
                yield p
            # remove duplicates generated by cartesian
            else:
                counter = counter + 1

    def replacer(self, subdomains):
        """Replace `-` and `_` found in subdomain with dots and vice versa"""
        log.debug("Replace underscores n dashes with dots")
        ...
        for s in subdomains:
            ...
            log.debug(_arr)
            if len(_arr) == 1:
                log.debug("No underscores found!")
                _deque.append(_arr)
                log.debug(f"{_deque=}")
            else:
        ...
        # stdout only if there are underscores/dashes
        if counter > 0:
